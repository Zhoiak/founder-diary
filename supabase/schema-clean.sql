-- Supabase schema for Founder Diary (Clean Version)
...
drop policy if exists "integration_counters_crud_member" on integration_counters;
drop policy if exists "key_results_update_goal_owner" on key_results;
drop policy if exists "weekly_reviews_crud_self_member" on weekly_reviews;
drop policy if exists "investor_updates_crud_self_member" on investor_updates;
drop policy if exists "investor_updates_public_read" on investor_updates;
...
-- integration counters (GitHub, etc.)
create table if not exists integration_counters (
  id uuid primary key default gen_random_uuid(),
  project_id uuid references projects(id) on delete cascade,
  integration text not null, -- 'github', 'linear', etc.
  metric text not null, -- 'commits', 'prs', 'issues', etc.
  count int default 0,
  date date not null,
  unique(project_id, integration, metric, date)
);
